...
Toast/theme UI (`sonner`, `next-themes`) is not pre-wired. Add via `pnpm dlx shadcn@latest add sonner` when you need it, and mount `ThemeProvider` + `Toaster` in the root layout.
