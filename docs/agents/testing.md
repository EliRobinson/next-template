...
- Assert behavior with roles and text. Never compare screenshots here; see Screenshot regression below.
...
- `scripts/visual-test.sh` starts Linux Chromium in the Playwright Docker image and runs the tests against it, so every machine renders the same pixels. The config refuses a run on a local macOS or Windows browser. Never make baselines any other way.
...
- The pre-push hook runs `pnpm test:visual`.
...
- Both themes (`data-theme` light and dark) for any component with color.
...
- Full pages, E2E flows, or integration tests. They pull in live data, fonts loading late, and layout from many components, so they drift on every unrelated change. `e2e/` specs never call `toHaveScreenshot()`.
...
- Set the viewport with `test.use({ viewport })` so layout does not depend on the default.
